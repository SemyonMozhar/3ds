...
4. Дождитесь окончания создания бекапа.
2. Нажмите (SELECT), чтобы извлечь карту памяти и вставьте ее в ПК.
8. Скопируйте `NANDmin_emu.bin` и `NANDmin_emu.bin.sha` из папки `/files9/` в безопасное место; сделайте резервные копии на разных носителях; этот бекап может спасти вам консоль от брика, в случае каких-либо неверных действий. **Ваш бекап должен соответствовать указанным размерам. Если это не так, то удалите его и сделайте по новой**.
