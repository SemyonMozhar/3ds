...
Если перед понижением прошивки до 2.1.0 на 2DS или New 3DS вы забыли включить Wi-Fi, вытащите батарею и отключите зарядное устройство на несколько секунд, и затем снова включите консоль.
{: .notice--info}
...
**В процессе ctrtransfer с устройства будут удалены все установленные пользователем ticket-файлы до тех пор, пока резервная копия не будет восстановлена. НИ В КОЕМ СЛУЧАЕ не пропускайте создание бэкапа!**
{: .notice--danger}
...
2. Скопируйте `.bin` и `.bin.sha` из `.zip-архива` 2.1.0 ctrtransfer в папку `/files9/` на SD-карте
...
3. Если у вас нет NAND бэкапа, **создайте бэкап SysNAND в файле `NANDmin.bin` при появлении запроса "Optional NAND backup" нажатием кнопки (А)**
