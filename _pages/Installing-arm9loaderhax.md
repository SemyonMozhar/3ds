...
* Предыдущая версия [SafeA9LHInstaller](https://github.com/AuroraWright/SafeA9LHInstaller/releases/tag/v2.0.3) *(`.7z`-файл)*
...
5. Если ваш бекап был сделан на прошивках от 3.0.0 до 4.5.0, консольне запустится до тех пор, пока вы вручную не выкачаете нужные файлы прошивки:
...
16. Запустите Luma3DS Updater.
17. Выберите курсором пункт Install Normal Version.
18. Нажмите (A).
19. Нажмите (START) для перезагрузки.


##### <a name="part7" />Часть VII - Восстановление тикетов
...
Если вы [перемещали EmuNAND](move-emunand), пропустите эту часть.
{: .notice--info}

Если вы делали бекап в процессе понижения с DSiWare или с помощью хардмода, пропустите эту часть.
{: .notice--info}

Если у вас нет тикетов, или нет приложений, которые хотелось бы восстановить, пропустите эту часть.
{: .notice--info}
...
##### <a name="part8" />Часть VIII - Восстановление nim

Эта часть необходима только тем, кто делал [даунгрейд до 9.2.0](9.2.0 Dowgnrade).
{: .notice--info}

1. Запустите GodMode9 из arm9loaderhax, зажав (ВВЕРХ) во время загрузки.
2. Перейдите в папку `SDCARD`.
3. Нажмите (Y) на `nimsavegame.bin`.
4. Нажмите (B) для возврата в главное меню программы.
5. Перейдите по пути `SYSNAND CTRNAND` -> `data` -> (32-х значный ID) -> `sysdata` -> `0001002c`.
7. Нажмите (A) на пункте `00000000`.
8. Выберите "Inject data @offset".
9. Нажмите (A), чтобы выбрать смещение `00000000`.
10. Нажмите (A) для для того, чтобы разрешить программе писать в SysNAND, а затем введите указанную комбинацию клавиш.
