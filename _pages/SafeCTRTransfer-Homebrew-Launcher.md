...
Если вы использовали Управление microSD (microSD Management) для передачи данных на New 3DS, вы не сможете им воспользоваться на прошивке 2.1.1. Убедитесь, что у вас есть картридер для microSD-карт, прежде чем продолжить.
{: .notice--info}
...
1. Скопируйте `fasthax.3dsx` в папку `/3ds/` на SD-карте
...
1. Выберите fasthax в списке homebrew
  + Вам может потребоваться пролистать список вниз, чтобы увидеть нужный пункт
1. После завершения, нажмите (START) для выхода из fasthax
...
##### <a name="part2" />Часть III - CTRTransfer
